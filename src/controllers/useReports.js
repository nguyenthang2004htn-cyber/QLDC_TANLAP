...
const useReports = (managedArea = '', citizenId = '') => {
  ...
  const updateStatusDirectly = async (id, nextStatus, message = null) => {
    try {
      await updateReportStatus(id, nextStatus, message);
      setReports((prev) =>
        prev.map((r) => ({
      ...
      console.error('Lỗi cập nhật trạng thái phản ánh trực tiếp:', error);
    }
  };
  ...
  const mappedReports = reports.map(r => ({
    ...r,
    urgent: r.khan_cap === 1 || r.khan_cap === true || r.loai === 'urgent' || r.hinh_thuc === 'Khẩn cấp',
  }));

  const visibleReports = getFilteredByArea(mappedReports, managedArea);
