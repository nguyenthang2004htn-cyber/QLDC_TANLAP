    ...
    <>
      ...
      <aside className={`sidebar ${mobileOpen ? 'mobile-open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          ...
          {user.role === 'citizen' && (
            <>
              ...
              <NavLink to="/citizen?tab=hogiadinh" onClick={handleNavClick} className={getNavClass('hogiadinh', null)}>Khai Báo Hộ Gia Đình</NavLink>
          ...
          {user.role === 'official' && (
            <>
              ...
              <NavLink to="/official?tab=hogiadinh" onClick={handleNavClick} className={getNavClass('hogiadinh', null)}>Quản lý Hộ Gia Đình</NavLink>
          ...
          {user.role === 'office' && (
            <>
              <div style={{ padding: '0.5rem 1.5rem', fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--gray-600)', letterSpacing: '0.05em', fontWeight: 600, marginTop: '0.5rem' }}>Nghiệp vụ Văn phòng</div>
              <NavLink to="/office?tab=reports" onClick={handleNavClick} className={getNavClass('reports', 'reports')}>Tiếp nhận & Xử lý Phản ánh</NavLink>
              <NavLink to="/office?tab=announcements" onClick={handleNavClick} className={getNavClass('announcements', null)}>Đăng tải Thông báo</NavLink>
              <NavLink to="/office?tab=map" onClick={handleNavClick} className={getNavClass('map', null)}>Bản đồ Sự cố</NavLink>
            </>
          )}
          ...
          {user.role === 'admin' && (
            <>
              ...
              <NavLink to="/admin?tab=announcements" onClick={handleNavClick} className={getNavClass('announcements', null)}>Đăng tải Thông báo</NavLink>
            </>
          )}
        ...
        <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid var(--gray-200)' }}>
          ...
          <span className={`badge ${user.role === 'superadmin' ? 'badge-danger' : user.role === 'admin' ? 'badge-processing' : ['official', 'office'].includes(user.role) ? 'badge-verifying' : 'badge-completed'}`}>
            {user.roleName}
          </span>
        </div>
      </aside>
