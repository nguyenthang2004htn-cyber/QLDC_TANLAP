                ...
                {allUsers.map(u => (
                  <tr key={u.id} style={{ borderBottom: '1px solid var(--gray-200)' }}>
                    ...
                    <td style={{ padding: '1rem 0' }}>
                      <span className={`badge ${u.vai_tro === 'superadmin' ? 'badge-danger' : u.vai_tro === 'admin' ? 'badge-processing' : u.vai_tro === 'official' ? 'badge-verifying' : 'badge-completed'}`}>
                        {u.vai_tro}
                      </span>
                    </td>
                      ...
                      {u.vai_tro !== 'superadmin' && (
                        <>
                          <button className="btn btn-outline" style={{ padding: '0.25rem 0.5rem', marginRight: '0.5rem', fontSize: '0.8rem' }} onClick={async () => {
                            const newRole = window.prompt("Nhập vai trò mới (citizen, official, admin):", u.vai_tro);
                            if (newRole && ['citizen', 'official', 'admin'].includes(newRole)) {
